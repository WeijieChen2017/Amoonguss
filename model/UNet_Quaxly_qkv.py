...
from .convolutions_qkv import Convolution, ResidualUnit
...
__all__ = ["UNet_Quaxly_qkv"]


@export("monai.networks.nets")
@alias("Unet")
class UNet_Quaxly_qkv(nn.Module):
    """
    ...
    def forward(self, x: torch.Tensor, is_deep_supervision = False,) -> torch.Tensor:
        ...
        x4 = self.down4(x3)
        xb = self.bottom(x4)
        x5 = self.up4(torch.cat([x4, xb], dim=1))
        x6 = self.up3(torch.cat([x3, x5], dim=1))
        x7 = self.up2(torch.cat([x2, x6], dim=1))
        x8 = self.up1(torch.cat([x1, x7], dim=1))

        if is_deep_supervision:
            ds_1 = self.ds_out_1(torch.cat([x1, x7], dim=1))
