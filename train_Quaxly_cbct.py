...
model_list = [
    ["Quaxly_cbct", [1], 912, 4, 0],
    ["Quaxly_cbct", [0], 912, 6, 1],
    ["Quaxly_cbct", [3], 912, 6, 2],
    ["Quaxly_cbct", [3], 912, 6, 3],
]
...
train_transforms = Compose(
    [
        LoadImaged(keys=["CBCT", "CT", "MASK"]),
        EnsureChannelFirstd(keys=["CBCT", "CT", "MASK"]),
        Orientationd(keys=["CBCT", "CT", "MASK"], axcodes="RAS"),
        Spacingd(
            keys=["CBCT", "CT", "MASK"],
            pixdim=(1., 1., 1.),
...
for idx_epoch_new in range(train_dict["train_epochs"]):
    ...
    for step, batch in enumerate(train_loader):
        mr, ct, mask_mr = (batch["CBCT"].float().to(device), batch["CT"].float().to(device), batch["MASK"].float().to(device))
