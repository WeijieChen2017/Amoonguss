...
def create_nfold_json_v3b(data_json, nfold, random_seed, save_dir):
    # Load the JSON data
    with open(data_json, "r") as f:
        data = json.load(f)
    ...
    case_ids = []
    labels = []
    for key in ["MR", "CT", "MASK", "v3a_last"]:
        for filepath in data[key]:
            case_id = os.path.dirname(filepath)
            if case_id not in case_ids:
                case_ids.append(case_id)
    ...
    # Function to group files by case
    def group_files_by_case(files, cases):
        case_dict = {case: {"MR": None, "CT": None, "MASK": None, "v3a_last": None} for case in cases}
        for filepath in files:
            case_id = os.path.dirname(filepath)
            file_type = os.path.basename(filepath).lower()
            if file_type == "mr.nii.gz":
                ...
                case_dict[case_id]["MASK"] = filepath
            elif file_type == "sct_v3a_last.nii.gz":
                case_dict[case_id]["v3a_last"] = filepath
        return list(case_dict.values())
...
def create_nfold_json_MRMASK(data_json, nfold, random_seed, save_dir):
    ...
    # Function to group files by case
    def group_files_by_case(files, cases):
        case_dict = {case: {"MR": None, "CT": None, "MASK_MR": None} for case in cases}
        for filepath in files:
            case_id = os.path.dirname(filepath)
            file_type = os.path.basename(filepath).lower()
            if file_type == "mr.nii.gz":
                ...
                case_dict[case_id]["CT"] = filepath
            elif file_type == "mask_mri_th60.nii.gz":
                case_dict[case_id]["MASK_MR"] = filepath
        return list(case_dict.values())
