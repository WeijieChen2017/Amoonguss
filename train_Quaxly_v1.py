...
for idx_epoch_new in range(train_dict["train_epochs"]):
    ...
    for step, batch in enumerate(train_loader):
        step += 1
        mr, ct, mask = (batch["MR"].to(device), batch["CT"].to(device), batch["MASK"].to(device))
    ...
    if (idx_epoch+1) % train_dict["val_interval"] == 0:
        ...
        if best_mae < best_val_loss:
            ...
            torch.save(model.state_dict(), train_dict["save_folder"]+"model/model_best_{:04d}.pth".format(idx_epoch+1))
            torch.save(optim.state_dict(), train_dict["save_folder"]+"model/optim_best_{:04d}.pth".format(idx_epoch+1))
    ...
    if (idx_epoch+1) % train_dict["save_per_epochs"] == 0:
        torch.save(model.state_dict(), train_dict["save_folder"]+"model/model_{:04d}.pth".format(idx_epoch+1))
        torch.save(optim.state_dict(), train_dict["save_folder"]+"model/optim_{:04d}.pth".format(idx_epoch+1))
